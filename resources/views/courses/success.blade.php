...
<div class="max-w-3xl mx-auto space-y-8 py-4" x-data="{ copied: false, copyText(text) { navigator.clipboard.writeText(text); this.copied = true; setTimeout(() => this.copied = false, 2000); } }">
    ...
    {{-- Account Notification Banner (Luôn hiển thị đầy đủ Email và Mật khẩu là Số điện thoại) --}}
    @php
        $loginEmail = $registration->email ?: ($registration->user?->email ?? '');
        $initialPassword = $registration->phone_normalized ?: $registration->phone;
    @endphp
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 sm:p-7 shadow-sm space-y-5">
        <div class="flex items-start gap-3.5">
            ...
            <div class="min-w-0 flex-1">
                ...
                <p class="text-xs text-slate-500 mt-1 leading-relaxed">
                    Hệ thống tự động sử dụng Email và Số điện thoại bạn vừa điền để làm tài khoản học viên. Bạn có thể sử dụng thông tin này để đăng nhập ôn Flashcard 3D, làm bài kiểm tra HSK và theo dõi lớp học.
                </p>
            </div>
        </div>

        {{-- Account Credentials Box (Email + Mật khẩu luôn hiển thị) --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5 flex items-center justify-between">
                <div class="min-w-0 pr-2">
                    ...
                    <span class="text-sm font-black font-mono text-slate-900 truncate block mt-0.5">{{ $loginEmail }}</span>
                </div>
                <button @click="copyText('{{ $loginEmail }}')" type="button" title="Sao chép email"
            ...
            <div class="rounded-2xl border border-amber-200 bg-amber-50/50 p-3.5 flex items-center justify-between">
                <div class="min-w-0 pr-2">
                    ...
                    <span class="text-sm font-black font-mono text-amber-950 truncate block mt-0.5">{{ $initialPassword }}</span>
                </div>
                <button @click="copyText('{{ $initialPassword }}')" type="button" title="Sao chép mật khẩu"
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('profile.edit') }}#my-courses" class="inline-flex items-center gap-1.5 rounded-xl bg-slate-950 px-4 py-2 text-xs font-bold text-white hover:bg-slate-800 transition shadow-sm">
                    <i data-lucide="bookmark-check" class="h-3.5 w-3.5 text-amber-400"></i>
                    <span>Xem "Khóa học của tôi" trong Hồ sơ</span>
                </a>
