...
<div class="max-w-3xl mx-auto space-y-8 py-4" x-data="{ copied: false, copyText(text) { navigator.clipboard.writeText(text); this.copied = true; setTimeout(() => this.copied = false, 2000); } }">
    ...
    <div class="rounded-2xl bg-emerald-50 border border-emerald-200 p-4 flex items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-xs font-bold text-emerald-900">Đơn đã được lưu vào tài khoản của bạn</p>
                <p class="text-[11px] text-emerald-700 mt-0.5">Xem lại bất cứ lúc nào trong "Khóa học của tôi"</p>
            </div>
        </div>
        <a href="{{ route('courses.my') }}"
           class="shrink-0 inline-flex items-center gap-1.5 rounded-xl bg-emerald-700 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-800 transition">
            Xem ngay →
        </a>
    </div>
    @else
    <div class="rounded-2xl bg-blue-50 border border-blue-200 p-5 space-y-3">
        <div class="flex items-start gap-3">
            <div class="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-blue-100 text-blue-700 mt-0.5">
                <i data-lucide="user-plus" class="h-5 w-5"></i>
            </div>
            <div>
                <p class="text-sm font-bold text-blue-900">Lưu đơn đăng ký vào tài khoản?</p>
                <p class="text-xs text-blue-700 mt-1 leading-relaxed">
                    Tạo tài khoản miễn phí để theo dõi trạng thái đơn đăng ký, xem lại QR thanh toán và nhận thông báo
                    khi giáo viên xác nhận lớp học. Bạn cũng có thể dùng web học tiếng Trung miễn phí!
                </p>
            </div>
        </div>
        <div class="flex flex-col sm:flex-row gap-2 pl-12">
            <a href="{{ route('register') }}"
               class="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-700 px-4 py-2.5 text-xs font-bold text-white hover:bg-blue-800 transition shadow-sm">
                <i data-lucide="user-plus" class="h-3.5 w-3.5"></i>
                Tạo tài khoản miễn phí
            </a>
            <a href="{{ route('login') }}"
               class="inline-flex items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white px-4 py-2.5 text-xs font-bold text-blue-800 hover:bg-blue-50 transition">
